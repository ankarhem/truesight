        ...
        modules: vec![ModuleInfo {
            ...
            symbols: vec![
                ...
                SymbolInfo {
                    name: "validate_token".to_string(),
                    kind: CodeUnitKind::Function,
                    signature: "pub fn validate_token(token: &str) -> Result<Claims>".to_string(),
...
fn search_config_round_trips_through_json() {
    ...
    let deserialized: SearchConfig = serde_json::from_str(&json).expect("Failed to deserialize");
...
fn index_metadata_round_trips_through_json() {
    ...
    let deserialized: IndexMetadata = serde_json::from_str(&json).expect("Failed to deserialize");
